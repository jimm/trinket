...
static set<InputInstrument *> inputs;
...
// While the InputInstrument pointed to by `in_voidptr` is running, take PmMessages
...
void *read_thread(void *in_voidptr) {
  InputInstrument *in = (InputInstrument *)in_voidptr;
  struct timespec rqtp = {0, SLEEP_NANOSECS};
...
InputInstrument::InputInstrument(sqlite3_int64 id, PmDeviceID device_id, const char *device_name, const char *name)
...
// starts a `read_thread` for this InputInstrument.
void InputInstrument::start() {
  Instrument::start();
...
// InputInstrument to exit. Also removes the InputInstrument from `inputs`.
void InputInstrument::stop() {
  running = false;
  read_pthread = 0;
  inputs_mutex.lock();
  for (set<InputInstrument *>::iterator i = inputs.begin(); i != inputs.end(); ++i) {
    if (*i == this) {
      inputs.erase(i);
      break;
    }
...
bool InputInstrument::start_midi() {
...
// manner.
void InputInstrument::enqueue(PmEvent *events, int num) {
...
void InputInstrument::read(PmMessage msg) {
  if (!enabled && real_port())
    return;
  ...
  pipe->process(Pm_MessageStatus(msg), Pm_MessageData1(msg),
                Pm_MessageData2(msg), (msg >> 24) & 0xff);
}

// Removes a single PmMessages from our message queue in a thread-safe
// manner and returns it.
PmMessage InputInstrument::message_from_read_queue() {
  PmMessage msg = 0;
  message_queue_mutex.lock();
